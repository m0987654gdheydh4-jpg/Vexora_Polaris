...
float superAlpha(vec2 size, vec2 coord, vec4 radius, float smoothness, float cornerSmoothness) {
    vec2 center = size * 0.5;
    vec2 halfSize = max(center - 1.0, vec2(0.0));
    vec2 pos = center - coord * size;
    float dist = roundedBoxSDF(-pos, halfSize, radius, cornerSmoothness);
    return 1.0 - smoothstep(1.0 - smoothness, 1.0, dist);
}

float outlineAlpha(vec2 coord, vec2 size, vec4 radius, float thickness, float smoothness, float cornerSmoothness) {
    float outer = superAlpha(size, coord, radius, smoothness, cornerSmoothness);
    ...
    float inner = superAlpha(innerSize, innerCoord, innerRadius, smoothness, cornerSmoothness);
...
void main() {
    ...
    float alpha = outlineAlpha(coord, size, radius, thickness, smoothness, cornerSmoothness);

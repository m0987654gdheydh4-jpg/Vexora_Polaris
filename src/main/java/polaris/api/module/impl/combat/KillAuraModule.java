...
public class KillAuraModule extends Module {
    ...
    public final MultiModeSetting options = register(new MultiModeSetting("Options", "KillAura options.", new String[]{"Pause While Using", "Ignore Walls", "Release Shield", "Sync TPS"}, "Pause While Using"));
    ...
    // Old Delay settings (как в toffi)
    public final BooleanSetting oldDelay = register(new BooleanSetting("Old Delay", "Use old 1.8 delay system with CPS.", false));
    public final BooleanSetting highCps = register(new BooleanSetting("High CPS", "Allow CPS up to 200 instead of 20.", false));
    public final NumberSetting minCps = register(new NumberSetting("Min CPS", "Minimum clicks per second.", 10.0, 1.0, 20.0, 1.0));
    public final NumberSetting maxCps = register(new NumberSetting("Max CPS", "Maximum clicks per second.", 14.0, 1.0, 20.0, 1.0));
    ...
    // CPS control (как в toffi)
    public long lastAttackTime = 0L;
    public long currentClickDelay = 0L;

    public KillAuraModule() {
        super("KillAura", "Automatically attacks nearby targets.", ModuleCategory.COMBAT);
        ...
        // Old Delay visibility (как в toffi)
        minCps.visibleWhen(oldDelay::getValue);
        maxCps.visibleWhen(oldDelay::getValue);
        highCps.visibleWhen(oldDelay::getValue);

        instance = this;
    }

    public static KillAuraModule getInstance() {
        return instance;
    }
    ...
    // Old Delay getters
    public boolean isOldDelayEnabled() {
        return oldDelay.getValue();
    }

    public boolean isHighCpsEnabled() {
        return highCps.getValue();
    }

    public double getMinCps() {
        double cps = minCps.getValue();
        if (!highCps.getValue()) {
            return Math.min(cps, 20.0);
        }
        return cps;
    }

    public double getMaxCps() {
        double cps = maxCps.getValue();
        if (!highCps.getValue()) {
            return Math.min(cps, 20.0);
        }
        return cps;
    ...
    // ===== ЛОГИКА УДАРОВ КАК В TOFFI =====

    // Проверяет только криты (без кулдауна)
    private boolean checkCritStatusOnly() {
        if (!onlyCriticals.getValue()) return true;
        Minecraft client = Minecraft.getInstance();
        if (client.player == null) return true;
        if (!client.options.keyJump.isDown()) return true;
        boolean inBadPosition = client.player.isInWater() || client.player.isInLava() || client.player.onClimbable();
        return !inBadPosition ? (client.player.fallDistance > 0.0f && !client.player.onGround()) : true;
    }

    // Проверяет кулдаун предмета (1.12+ логика)
    private boolean canCritAttack() {
        Minecraft client = Minecraft.getInstance();
        if (client.player == null) return false;
        // getCooledAttackStrength(1.0f) - ждёт полный кулдаун предмета
        // 0.92f - порог как в toffi (почти полный кулдаун)
        if (client.player.getAttackStrengthScale(1.0f) < 0.92f) return false;
        return checkCritStatusOnly();
    }

    // Проверяет CPS задержку (1.8 логика)
    private boolean canCpsAttack() {
        long currentTime = System.currentTimeMillis();
        return (currentTime - lastAttackTime) >= currentClickDelay;
    }

    // Обновляет CPS задержку со случайным CPS между min и max
    private void updateCpsDelay() {
        int min = (int) Math.min(getMinCps(), getMaxCps());
        int max = (int) Math.max(getMinCps(), getMaxCps());
        int randomCps = min + (max - min <= 0 ? 0 : random.nextInt(max - min + 1));
        currentClickDelay = 1000L / (randomCps <= 0 ? 1 : randomCps);
    }
    ...
    // ===== ГЛАВНАЯ ЛОГИКА АТАКИ КАК В TOFFI =====
    private void performAuraAttack(StrikerConstructor.AttackPerpetratorConfigurable config) {
        ...
        if (oldDelay.getValue()) {
            // Old Delay ВКЛ - бьём с CPS (1.8 логика)
            // Меч/рука (1.6-1.8 скорость) → бьёт быстро, например 100 CPS
            if (System.currentTimeMillis() - lastAttackTime >= currentClickDelay) {
                if (checkCritStatusOnly()) {
                    canAttack = true;
                    updateCpsDelay(); // новый случайный CPS между min и max
                }
            }
        } else {
            // Old Delay ВЫКЛ - ждём кулдаун предмета (1.12+ логика)
            // Топор (1.0-1.2 скорость) → бьёт раз в ~1 сек
            if (canCritAttack()) {
                canAttack = true;
            }
        }

package polaris.utils.render.ui.ripple;
...
public final class RippleRenderer implements AutoCloseable {
    ...
    public static final RenderPipeline RIPPLE_PIPELINE = RenderPipeline.builder()
            .withLocation(id("pipeline/ripple"))
            .withVertexShader(id("ui/ripple/ripple"))
            .withFragmentShader(id("ui/ripple/ripple"))
            ...
            .withUniform("RippleParamsArray", com.mojang.blaze3d.shaders.UniformType.UNIFORM_BUFFER)
            .build();
    ...
    private void submit(GuiGraphics graphics, BuiltRipple ripple) {
        if (graphics == null || ripple == null || !ripple.visible()) {
            return;
        }
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor) graphics)
                    ...
                    .submitGuiElement(new RippleRenderState(pose, ripple, ScissorUtil.current()));
        } catch (RuntimeException ignored) {
        }
    }
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer uniformData = buildUniformData(stack, preparedRipples);
            paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "cataclysm_ripple_params", GpuBuffer.USAGE_UNIFORM, uniformData);
            paramsDirty = false;
            return paramsBuffer;
        }
    }

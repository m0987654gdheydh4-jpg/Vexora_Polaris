...
public final class ImageRenderer implements AutoCloseable {
    ...
    private void submit(GuiGraphics graphics, BuiltImage image) {
        ...
        try {
            GuiRenderState guiState = ((GuiGraphicsExtractorAccessor) graphics).cataclysm$getGuiRenderState();
            int layerSerial = ((GuiRenderStateLayerAccessor) guiState).cataclysm$getLayerSerial();
    ...
    public static void invalidateTexture(Identifier id) {
        if (id == null) return;
        ImageRenderer renderer = getInstance();
        renderer.textures.entrySet().removeIf(e -> id.equals(e.getKey().id()));
    }
    ...
    private ImageTexture resolveTexture(String path, boolean nearestFilter) {
        ...
        if (texture == null || texture.getTextureView() == null || texture.getTexture() == null) {
            return null;
        }
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer uniformData = buildUniformData(stack);
            paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "cataclysm_image_params", GpuBuffer.USAGE_UNIFORM, uniformData);
            paramsDirty = false;
            return paramsBuffer;
        }
    }

package polaris.utils.render.ui.glass;
...
public final class GlassRenderer implements AutoCloseable {
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer uniformData = buildUniformData(stack, preparedGlasses);
            paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "cataclysm_glass_params", GpuBuffer.USAGE_UNIFORM, uniformData);
            paramsDirty = false;
            return paramsBuffer;
        }
    }

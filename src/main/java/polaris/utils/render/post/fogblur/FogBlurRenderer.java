package polaris.utils.render.post.fogblur;
...
public final class FogBlurRenderer {
    ...
    private static final Identifier KAWASE_DOWN_PIPELINE_ID = Identifier.fromNamespaceAndPath("cataclysm", "pipeline/post/fogblur/kawase_down");
    private static final Identifier KAWASE_UP_PIPELINE_ID = Identifier.fromNamespaceAndPath("cataclysm", "pipeline/post/fogblur/kawase_up");
    private static final Identifier COMPOSITE_PIPELINE_ID = Identifier.fromNamespaceAndPath("cataclysm", "pipeline/post/fogblur/composite");
    private static final Identifier KAWASE_VERTEX = Identifier.fromNamespaceAndPath("cataclysm", "post/fogblur/kawase");
    private static final Identifier KAWASE_DOWN_SHADER = Identifier.fromNamespaceAndPath("cataclysm", "ui/kawase/down");
    private static final Identifier KAWASE_UP_SHADER = Identifier.fromNamespaceAndPath("cataclysm", "ui/kawase/up");
    private static final Identifier COMPOSITE_SHADER = Identifier.fromNamespaceAndPath("cataclysm", "post/fogblur/composite");
        ...
        try {
            ...
            if (compositePipeline == null) {
                compositePipeline = RenderPipelines.register(
                        RenderPipeline.builder()
                                ...
                                .withVertexShader(Identifier.fromNamespaceAndPath("cataclysm", "post/fogblur/fogblur"))
            ...
            if (kawaseUniformBuffer == null || kawaseUniformBuffer.isClosed() || kawaseUniformBuffer.size() < KAWASE_UNIFORM_SIZE) {
                ...
                kawaseUniformBuffer = RenderSystem.getDevice().createBuffer(
                        () -> "cataclysm:fog_blur_kawase_uniforms",
                        GpuBuffer.USAGE_UNIFORM | GpuBuffer.USAGE_COPY_DST,
                        KAWASE_UNIFORM_SIZE
                );
            }
    ...
    private static boolean ensureTargets(int width, int height, float renderScale, int passes) {
        ...
        for (int i = 0; i < passes; i++) {
            downTargets[i] = ensureTarget(downTargets[i], "cataclysm_fog_blur_down_" + i, currentWidth, currentHeight);
            if (i < passes - 1) {
                upTargets[i] = ensureTarget(upTargets[i], "cataclysm_fog_blur_up_" + i, currentWidth, currentHeight);
            }
    ...
    private static void renderKawasePass(RenderPipeline pipeline, GpuTextureView source, int sourceWidth, int sourceHeight, TextureTarget target, float offsetScale, float sourceX, float sourceY, float sourceW, float sourceH, GpuSampler sampler) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            try (RenderPass renderPass = encoder.createRenderPass(() -> "cataclysm:fog_blur_kawase", target.getColorTextureView(), OptionalInt.of(ColorUtil.TRANSPARENT))) {
    ...
    private static void composite(GpuTextureView target, GpuTextureView scene, GpuTextureView blur, GpuTextureView depth, float opacity, float farPlane, float minThreshold, float maxThreshold) {
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "cataclysm:fog_blur_composite", target, OptionalInt.empty())) {

...
import polaris.mixin.accessor.MixinRenderPipelines;
...
public final class Render3D implements IMinecraft {
    ...
    private static final RenderPipeline.Snippet cataclysm_lines_SNIPPET = RenderPipeline.builder(MixinRenderPipelines.cataclysm$getLinesSnippet())
            .withBlend(STANDARD_BLEND)
            .withDepthWrite(false)
            .withCull(false)
            .buildSnippet();

    private static final RenderType cataclysm_lines_OVERLAY = RenderLayerFactory.create(
            "rendertype/cataclysm_lines_overlay",
            256,
            RenderPipeline.builder(cataclysm_lines_SNIPPET)
                    .withLocation("pipelines/cataclysm_lines_overlay")
                    .withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
                    .build()
    );

    private static final RenderType cataclysm_lines_NO_DEPTH = RenderLayerFactory.create(
            "rendertype/cataclysm_lines_no_depth",
            256,
            RenderPipeline.builder(cataclysm_lines_SNIPPET)
                    .withLocation("pipelines/cataclysm_lines_no_depth")
                    .withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
                    .build()
    );

    private static final RenderType cataclysm_filled_BOX_NO_DEPTH = RenderLayerFactory.create(
            "rendertype/cataclysm_filled_box_no_depth",
            256,
            RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
                    .withLocation("pipelines/cataclysm_filled_box_no_depth")
                    .withBlend(STANDARD_BLEND)
                    .withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST)
                    .withDepthWrite(false)
                    .withCull(false)
    ...
    private static void renderLines(PoseStack matrices, MultiBufferSource.BufferSource immediate, Vec3 cameraPos) {
        ...
        try {
            ...
            renderLineBatch(matrices, immediate, cameraPos, LINE, cataclysm_lines_NO_DEPTH);
            renderLineBatch(matrices, immediate, cameraPos, LINE_OVERLAY, cataclysm_lines_OVERLAY);
        } finally {
            LINE.clear();
            LINE_DEPTH.clear();
            LINE_OVERLAY.clear();
    ...
    private static void renderQuads(PoseStack matrices, MultiBufferSource.BufferSource immediate, Vec3 cameraPos) {
        ...
        if (!QUAD.isEmpty()) {
            VertexConsumer buffer = immediate.getBuffer(cataclysm_filled_BOX_NO_DEPTH);
            for (Quad quad : QUAD) {
                drawQuadVertex(matrices, buffer, quad, cameraPos);
            }
            immediate.endBatch(cataclysm_filled_BOX_NO_DEPTH);
        }
